...
describe('AK/SK canonical signing', () => {
  it('sorts and encodes query by RFC3986 without treating plus as space', () => {
    expect(canonicalQuery('z=1&fields=id,status&page=1&a=hello%20world&a=*~&plus=a+b'))
      .toBe('a=%2A~&a=hello%20world&fields=id%2Cstatus&page=1&plus=a%2Bb&z=1');
  });

  it('normalizes uri while preserving path separators', () => {
    expect(canonicalUri('/api/订单 1/detail')).toBe('/api/%E8%AE%A2%E5%8D%95%201/detail');
  });
...
describe('request encryption', () => {
  it('SM4-GCM envelope round-trips and uses the gateway ENCv1 shape', () => {
    const envelope = encryptSm4GcmEnvelope(enc.encode('{"name":"wen"}'), sm4Key, iv);
    const encoded = encodeEnvelope(envelope);
    const decoded = decodeEnvelope(encoded);
    const plain = decryptSm4GcmEnvelope(decoded, sm4Key);

    expect(encoded).toMatch(/^ENCv1.[A-Za-z0-9_-]+.[A-Za-z0-9_-]+.[A-Za-z0-9_-]+$/);
    expect(new TextDecoder().decode(plain)).toBe('{"name":"wen"}');
  });

  it('builds a whole-body encrypted request before signing', async () => {
    ...
    expect(JSON.parse(built.bodyText)).toEqual({ data: expect.stringMatching(/^ENCv1./) });
    expect(built.debug.canonicalRequest).toContain(await sha256Hex(enc.encode(built.bodyText)));
  });

  it('rejects RSA+SM4 field encryption because the gateway does not support it', async () => {
    await expect(buildGatewayRequest({
      method: 'POST',
